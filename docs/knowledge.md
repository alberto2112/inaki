...
The factory receives `(agent_config, global_config, embedder)` and must return an object that implements `IKnowledgeSource` (`inaki/kernel/ports/knowledge_port.py`). If the factory raises an exception, it is logged as WARNING and the remaining sources continue working.
...
| Ports (read-only + indexable) | `inaki/kernel/ports/knowledge_port.py` |
