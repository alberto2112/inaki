...
**File:** `inaki/kernel/domain/agent_context.py`
...
**Function:** `expand_includes` in `inaki/kernel/_turn_pipeline.py` (a pure free function, **not** part of the `AgentContext` value object — file I/O lives in the use-case layer, not in the domain).
...
2. The **per-user context** file (`~/.inaki/users/{channel}/...`, see `inaki/kernel/run_agent.py::_read_user_context`).
