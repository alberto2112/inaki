...
from inaki.channels.telegram.commands import TelegramCommandsMixin
from inaki.channels.telegram.group_flow import TelegramGroupFlowMixin
from inaki.channels.telegram.media import TelegramMediaMixin, _AlbumBuffer
...
class TelegramBot(
    ...
    def __init__(
        ...
        # outbound del canal. El bot solo declara QUÉ evento corresponde a cada flujo.
        self._broadcast_egress = BroadcastEgress(broadcast_emitter, settings.id, tg_cfg.emit)
    ...
    async def announce_back_online(self) -> None:
        """Aviso 'online' a los chats con backlog — lo invoca ``TelegramChannel.start``."""
