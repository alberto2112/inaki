...
def _bootstrap_uc(ctx: typer.Context) -> tuple["ISchedulerUseCase", "GlobalConfig"]:
    ...
    from inaki.cli._common import resolve_dirs

    config_dir, _ = resolve_dirs()
...
def _bootstrap_daemon_client(ctx: typer.Context) -> Any:
    ...
    """
    from inaki.cli._common import build_daemon_client, resolve_dirs

    config_dir, _ = resolve_dirs()

    client, _ = build_daemon_client(config_dir)
    return client
...
def run_cmd(
    ...
    """
    from inaki.cli._common import handle_daemon_errors, require_daemon

    client = _bootstrap_daemon_client(ctx)
    require_daemon(client)

    try:
        result = handle_daemon_errors(lambda: client.scheduler_run(task_id))
    except TaskNotFoundError:
        typer.echo(f"Error: task {task_id} not found", err=True)
        raise typer.Exit(code=1)
