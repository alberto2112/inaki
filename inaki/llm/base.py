...
class ResolvedLLMConfig(BaseModel):
    ...
    Vive en el módulo ``llm`` (no en ``inaki/config``): es el contrato de
    entrada que los providers declaran en SU módulo. La factory del wiring lo
    compone desde la config YAML — dirección legal (wiring → módulo); al revés
    jamás.
    """
...
class BaseLLMProvider(ILLMProvider):
    ...
    def stream(
        ...
        ``inaki.kernel.ports.llm_port.ILLMProvider.stream`` para el detalle
