...
from inaki.kernel.ports.embedding_cache_port import IEmbeddingCache
...
def instanciar_tool(tool_cls: type[ITool], *, config_store: IToolConfigStore) -> ITool:
    """Construye una tool respetando el Tool Config Protocol.

    Una clase que declara ``config_namespace`` recibe el store compartido como
    ``config_store``; el resto conserva el contrato de cero argumentos. Es la
    ÚNICA regla de instanciación de tools de terceros (extensiones), para que el
    protocolo viva en un solo sitio y no en cada punto de registro.
    """
    if getattr(tool_cls, "config_namespace", ""):
        return tool_cls(config_store=config_store)  # type: ignore[call-arg]
    return tool_cls()
