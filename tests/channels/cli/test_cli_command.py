...
def test_chat_command_no_instancia_app_container() -> None:
    ...
    with patch(
        "inaki.cli._common.build_daemon_client",
...
def test_chat_command_pasa_agent_id_al_runner() -> None:
    ...
    with patch(
        "inaki.cli._common.build_daemon_client", return_value=(mock_client, mock_global_config)
    ):
        with patch("inaki.channels.cli.runner.run_cli") as mock_run_cli:
            runner.invoke(app, ["chat", "--agent", "dev"])
