...
from inaki.channels.telegram.bot import BACK_ONLINE_NOTICE, TelegramBot
...
def _fast_throttle():
    ...
    with patch("inaki.channels.telegram.bot.BACK_ONLINE_NOTICE_DELAY_SEC", 0):
        yield
...
async def test_throttle_entre_envios():
    ...
    with patch("inaki.channels.telegram.bot.asyncio.sleep", new=AsyncMock()) as mock_sleep:
        await bot._announce_back_online(app)
