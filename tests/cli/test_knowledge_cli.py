...
class TestKnowledgeIndex:
    def test_index_source_known_exits_0(self, runner, docs_dir: Path, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "inaki.cli.knowledge._load_global_config",
                return_value=global_cfg,
            ),
            patch(
                "inaki.cli.knowledge._build_document_source",
...
class TestKnowledgeIngest:
    def test_ingest_ok(self, runner, docs_dir: Path, tmp_path: Path) -> None:
        ...
        with (
            patch("inaki.cli.knowledge._load_global_config", return_value=global_cfg),
            patch("inaki.cli.knowledge._build_document_source", return_value=mock_source),
        ):
    ...
    def test_ingest_path_inexistente_exits_2(self, runner, docs_dir: Path) -> None:
        ...
        global_cfg = _build_global_config_mock(docs_dir)
        with patch("inaki.cli.knowledge._load_global_config", return_value=global_cfg):
