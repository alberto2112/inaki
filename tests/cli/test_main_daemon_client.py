...
def test_build_daemon_client_returns_client_and_config(tmp_path: Path) -> None:
    ...
    from inaki.cli._common import build_daemon_client

    client, global_cfg = build_daemon_client(config_dir)
...
def test_chat_without_daemon_shows_error(tmp_path: Path) -> None:
    ...
    with patch("inaki.cli._common.resolve_dirs", return_value=(config_dir, agents_dir)):
        with patch("inaki.cli._common.build_daemon_client") as mock_build:
