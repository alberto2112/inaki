...
def test_global_config_transcription_es_opcional_y_default_none() -> None:
    gc = GlobalConfig(
        app=__import__("inaki.config", fromlist=["AppConfig"]).AppConfig(),
        llm=LLMConfig(),
        embedding=EmbeddingConfig(),
        memories=__import__("inaki.config", fromlist=["MemoriesConfig"]).MemoriesConfig(),
        chat_history=__import__("inaki.config", fromlist=["ChatHistoryConfig"]).ChatHistoryConfig(),
    )
