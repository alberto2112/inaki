...
from inaki.llm.deepseek import (
    DeepSeekProvider,
    _coerce_param,
    _has_dsml,
...
async def test_complete_recovers_dsml_without_retry() -> None:
    ...
    with patch("inaki.llm.openai_compatible.httpx.AsyncClient", return_value=client):
...
async def test_complete_retries_once_when_parse_fails() -> None:
    ...
    with (
        patch("inaki.llm.openai_compatible.httpx.AsyncClient", return_value=client),
        patch("inaki.llm.deepseek.asyncio.sleep", new=AsyncMock()) as sleep,
    ):
        result = await provider.complete([Message(role=Role.USER, content="leé mi mail")], "sys")
...
async def test_complete_strips_when_dsml_persists_after_retry() -> None:
    ...
    with (
        patch("inaki.llm.openai_compatible.httpx.AsyncClient", return_value=client),
        patch("inaki.llm.deepseek.asyncio.sleep", new=AsyncMock()),
    ):
        result = await provider.complete([Message(role=Role.USER, content="leé mi mail")], "sys")
