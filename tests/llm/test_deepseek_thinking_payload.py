...
async def test_complete_captures_reasoning_content_into_thinking() -> None:
    ...
    with patch("inaki.llm.openai_compatible.httpx.AsyncClient", return_value=fake_client):
...
async def test_complete_uses_configured_timeout_in_httpx_client() -> None:
    ...
    with patch("inaki.llm.openai_compatible.httpx.AsyncClient", return_value=fake_client) as ctor:
